...
const App = () => {
  ...
  // J'utilise useEffect pour exécuter une fonction juste aprés mon premier montage uniquement
  // juste aprés mon 1er render seulement, avec un tableau vide.
  // J'utilise useEffect pour appleler on fonction fetch !

  // Pour rappel, useEffect, sans tableau équivaut à un didMount et un didUpdate à chaque fois !
  // et si des variable sont présent dans le tableau, le use effect sera activé si une de ces variables changent
  useEffect(() => {
    fetchPost();
    fetchCategory();
  }, []);
